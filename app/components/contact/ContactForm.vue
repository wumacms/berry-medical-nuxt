...
<template>
  <div class="bg-light rounded-3xl p-8 lg:p-10 shadow-xs border border-gray-100">
    ...
    <div
      v-if="submitSuccess"
      class="bg-green-50 border border-green-200 rounded-2xl p-6 text-center space-y-3"
    >
      ...
      <button
        type="button"
        class="text-xs text-primary font-semibold hover:underline mt-2 cursor-pointer"
    ...
    <form v-else class="space-y-5" @submit.prevent="submitForm">
      ...
      <div>
        ...
        <textarea
          ...
          rows="4"
          placeholder="请简要描述您的项目需求，以便我们更好地为您服务"
          class="w-full bg-white border border-gray-200 rounded-xl px-4 py-3 text-sm text-dark placeholder:text-dark/30 focus:outline-none focus:border-primary focus:ring-2 focus:ring-primary/20 transition resize-none"
        ></textarea>
      </div>
